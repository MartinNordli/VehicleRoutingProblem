...
mod ga {
    ...
    pub fn perform_selection(
        ...
        match method {
            SelectionMethod::Tournament => tournament_selection(shared_pop, tournament_size),
            SelectionMethod::Ranked => ranked_selection(shared_pop),
            SelectionMethod::Roulette => roulette_selection(shared_pop),
            SelectionMethod::Elitist => elitist_selection(shared_pop),
        }
    }
...
mod island {
    ...
    pub fn run_island_model(
        ...
        let selection_methods = [
            ga::SelectionMethod::Tournament,  // Øy 0: Turneringsseleksjon
            ga::SelectionMethod::Ranked,      // Øy 1: Rangbasert seleksjon
            ga::SelectionMethod::Roulette,    // Øy 2: Ruletthjulseleksjon
            ga::SelectionMethod::Elitist      // Øy 3: Elitistisk seleksjon
        ];
        ...
        for gen in 0..num_generations {
            ...
            for island_idx in 0..islands.len() {
                ...
                // Compute shared fitness for current island
                let shared_pop = ga::compute_shared_fitness(island, instance, sigma_share, alpha);
                
                // Sort by shared fitness
                let mut indices: Vec<usize> = (0..shared_pop.len()).collect();
                indices.sort_by(|&a, &b| shared_pop[a].1.partial_cmp(&shared_pop[b].1).unwrap());
                
                // Best løsning er nå shared_pop[indices[0]].0
                if shared_pop[indices[0]].1 < best_fitness_overall {
                    best_fitness_overall = shared_pop[indices[0]].1;
                    best_solution_overall = Some(shared_pop[indices[0]].0.clone());
                ...
                if gen % 250 == 0 || gen == num_generations - 1 {
                    let raw = ga::fitness(&shared_pop[indices[0]].0, instance);
                    ...
                    println!(
                        ...
                        gen, island_idx, selection_method, shared_pop[indices[0]].1, raw
                    );
                ...
                if memetic_rate > 0.0 {
                    ...
                    if should_apply_memetic {
                        let adaptive_memetic_rate = memetic_rate * (0.5 + 0.5 * progress); // Øker gradvis
                        
                        let ls_count = (island_pop_size as f64 * adaptive_memetic_rate).ceil() as usize;
                        
                        // Applisér mer intensivt lokalsøk mot slutten
                        for i in 0..ls_count.min(island.len()) {
                            // Apply intra-route local search
                            local_search::apply_local_search(&mut island[i], instance);
                            
                            // Prøv inter-route forbedringer hyppigere i senere generasjoner
                            let inter_route_probability = 0.2 + 0.6 * progress; // Øker fra 20% til 80%
                            if i < ls_count / 3 || rand::random::<f64>() < inter_route_probability {
                                // Prøv flere ganger i senere generasjoner
                ...
                let mut new_population: Vec<Solution> = indices
                    .iter()
                    .take(elitism_count)
                    .map(|&i| shared_pop[i].0.clone())
                    .collect();
                
                // Create new offspring via crossover and mutation
                // using island-specific selection method
                let mut rng = rand::thread_rng();
                while new_population.len() < island_pop_size {
                    // Bruk øyas tildelte seleksjonsmetode
                    let parent1 = ga::perform_selection(&shared_pop, selection_method, tournament_size);
                    let parent2 = ga::perform_selection(&shared_pop, selection_method, tournament_size);
                    
                    let mut child = if rng.gen::<f64>() < crossover_rate {
                        // Randomly choose between crossover operators
            ...
            if gen > 0 && gen % migration_interval == 0 {
                ...
                // Ring topology: each island sends migrants to the next
            ...
            if generations_without_improvement >= stagnation_generations {
                println!("Early stopping due to {} generations without improvement", 
                         stagnation_generations);
                should_stop.store(true, Ordering::Relaxed);
            }
        }
        
        // Standardformattering for å returnere resultater
        (
...
fn main() {
    ...
    let filename = "train/train_8.json";
